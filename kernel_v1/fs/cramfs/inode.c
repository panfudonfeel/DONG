...
static void *cramfs_read_nand(struct super_block *sb, unsigned int offset, unsigned int len)
{
	...
	struct mtd_info *mtd=((struct cramfs_sb_info *)(sb->s_fs_info))->mtd;;

	unsigned mblock, moffset, pblock, maddr;

//printk("cramfs_read_nand offset %x len %x\n",offset,len);
	if (!len)
		return NULL;
...
{
	if( ((struct cramfs_sb_info *)(sb->s_fs_info))->nand)
	{
		//printk("cramfs_read_nand PAGE_CACHE_SIZE %x offset %x  len %x\n",PAGE_CACHE_SIZE,offset,len);
...
//	struct mtd_info *mtd = sb->s_mtd;
	struct mtd_info *mtd = ((struct cramfs_sb_info *)(sb->s_fs_info))->mtd;
	int *block_map = ((struct cramfs_sb_info *)(sb->s_fs_info))->block_map;
	int blocks = ((struct cramfs_sb_info *)(sb->s_fs_info))->nblock;
	int pblock = -1;
	int i;
...
//jkeqiang add it
	...
	((struct cramfs_sb_info *)(sb->s_fs_info))->nand=0;
	if(MAJOR(sb->s_dev) == MTD_BLOCK_MAJOR)
	{
		...
		if(mtd->type != MTD_NANDFLASH)
			goto  not_mtdblock;
		((struct cramfs_sb_info *)(sb->s_fs_info))->mtd=mtd;
		((struct cramfs_sb_info *)(sb->s_fs_info))->nand=1;

		blocks = (unsigned long)mtd->size/(unsigned long)mtd->erasesize;

		printk("mtd->size = %x blocks = %x\n",mtd->size,blocks);

		block_map = kmalloc(blocks*sizeof(int),GFP_KERNEL);
		for(i=0;i<blocks;i++)
			block_map[i] = -1;
		((struct cramfs_sb_info *)(sb->s_fs_info))->block_map=block_map;
		((struct cramfs_sb_info *)(sb->s_fs_info))->nblock=blocks;



		printk("cheaking bad block...............\n");
		for(i=0;i<640;i++)
		{	
			if( mtd->block_isbad(mtd,i*0x20000) )
				printk("\nbad block at %x\n",i*0x20000);
			else
				printk(".");
		}
